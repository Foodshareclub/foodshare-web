...
export const topPages = defineEndpoint("top_pages", {
  ...
  params: {
    date_from: p.dateTime().optional("2026-03-22T02:18:28.235Z"),
    date_to: p.dateTime().optional("2026-03-21T02:18:28.235Z"),
  ...
  nodes: [
    node({
      ...
      sql: `
        ...
        FROM page_views
        WHERE timestamp >= parseDateTimeBestEffort({{String(date_from, '2026-03-22T02:18:28.235Z', required=False)}})
          AND timestamp <= parseDateTimeBestEffort({{String(date_to, '2026-03-21T02:18:28.235Z', required=False)}})
        GROUP BY pathname
        ORDER BY views DESC
