...
try {
  ...
  for (const name of ["FloatingOrbs", "GradientBackground"]) {
    const source = await readFile(new URL(`../src/components/gpu/${name}.wgsl`, import.meta.url), "utf8");
    const background = effect(gpu, source, { set: { params: { time: 0, scroll: 0, resolution: size } } });
    ...
    for (const time of [0, 6]) {
      ...
      for (let index = 0; index < pixels.length; index += 4) {
        ...
        if (alpha === 255 || Math.max(pixels[index], pixels[index + 1], pixels[index + 2]) > alpha + 1) {
    ...
    if (!frames[0].some((value, index) => value !== frames[1][index])) throw new Error(`${name}: animation is frozen`);
