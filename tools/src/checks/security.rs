...
pub fn run(files: &[String]) -> Result<()> {
    ...
    // Filter out test files and tools directory - they contain mock credentials which are false positives
    let files: Vec<String> = files
        .into_iter()
        .filter(|f| !is_test_file(f) && !f.starts_with("tools/") && !f.starts_with("scripts/"))
        .collect();

    if files.is_empty() {
        print_success("No files to check (excluded: test files, tools/)");
        return Ok(());
    }
