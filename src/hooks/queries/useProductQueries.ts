...
// Request deduplication map with TTL - prevents duplicate in-flight requests
// and cleans up stale entries to avoid memory leaks
const pendingRequests = new Map<string, { promise: Promise<unknown>; timestamp: number }>();
const REQUEST_TTL = 10000; // 10 seconds

/**
 * Clean up stale requests that may have failed to self-cleanup
 */
...
async function fetchSearchProducts(
  ...
  const res = await fetch(
    `/api/products?search=${encodeURIComponent(query)}&type=${encodeURIComponent(type)}`
  );
...
export const productKeys = {
  ...
  search: (query: string, type: string) => [...productKeys.all, "search", query, type] as const,
...
export function useProducts(
  productType: string,
  options?: Omit<UseQueryOptions<InitialProductStateType[], Error>, "queryKey" | "queryFn">
) {
  return useQuery({
 ...
 */
export function useInfiniteProducts(productType: string, initialData?: InitialProductStateType[]) {
  ...
  const query = useInfiniteQuery({
          ...
          pages: [
            {
              data: initialData,
              nextCursor:
                initialData.length >= PAGINATION.DEFAULT_PAGE_SIZE
    ...
    // Note: Using Infinity means data is always considered fresh on first load
    initialDataUpdatedAt: hasInitialData ? Infinity : undefined,
  ...
  return useMutation({
    ...
    onSuccess: (result, variables) => {
      ...
      if (result.data) {
        ...
        }>({ queryKey: productKeys.infinite(variables.post_type) }, (oldData) => {
          if (!oldData) return oldData;
          return {
            ...oldData,
            pages: oldData.pages.map((page, index) =>
              index === 0 ? { ...page, data: [newProduct, ...page.data] } : page
            ),
          };
        });
      }
...
export function useUpdateProduct() {
  ...
  return useMutation({
    mutationFn: async ({
      ...
      if (productData.post_description)
        formData.set("post_description", productData.post_description);
      ...
      if (productData.is_active !== undefined)
        formData.set("is_active", String(productData.is_active));
    ...
    onMutate: async (variables) => {
      ...
      if (previousProduct) {
        queryClient.setQueryData(productKeys.detail(variables.id), {
          ...previousProduct,
          ...variables,
        });
      }
  ...
  return useMutation({
    ...
    onMutate: async (productId) => {
      ...
      queryClient.setQueriesData<{
        pages: PaginatedResponse[];
        pageParams: (number | null)[];
      }>({ queryKey: ["products", "infinite"] }, (oldData) => {
        if (!oldData) return oldData;
        return {
          ...oldData,
...
export function useToggleFavorite() {
  ...
  return useMutation({
    ...
    onMutate: async (variables) => {
      ...
      if (previousProduct) {
        queryClient.setQueryData(productKeys.detail(variables.productId), {
          ...previousProduct,
