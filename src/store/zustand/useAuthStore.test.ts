...
import { useAuthStore, authActions, selectUser, selectIsAuthenticated, selectIsAdmin } from './useAuthStore';
...
describe('useAuthStore', () => {
  ...
  describe('persistence security', () => {
    it('should only persist adminCheckStatus (non-sensitive data)', () => {
      // Set sensitive data
      act(() => {
        useAuthStore.getState().setAuth(mockUser, mockSession);
        useAuthStore.getState().setAdmin(true, ['admin']);
      ...
      // The partialize function is configured to only persist adminCheckStatus
      // We can verify this by checking the persist options
      const persistOptions = (useAuthStore as unknown as { persist: { getOptions: () => { partialize: (state: unknown) => unknown } } }).persist?.getOptions?.();
      ...
      if (persistOptions?.partialize) {
        ...
        // Should only contain adminCheckStatus
