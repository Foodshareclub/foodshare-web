...
class HealthManagementAPI {
  ...
  async getSystemDiagnostics(): Promise<SystemDiagnostics> {
    ...
    try {
      const [redisInfo, circuitBreakerKeys, metrics, performanceData] = await Promise.all([
        this.redis
          .ping()
          .then(() => "available")
          .catch(() => "unavailable"),
  ...
  private async getCircuitBreakerStates(keys: string[]): Promise<
    Array<{
      service: string;
      state: "open" | "closed" | "half-open";
      failure_count: number;
  ...
  async getMetricsExport(): Promise<string> {
    try {
      const [metrics, _perfData, healthKeys] = await Promise.all([
        this.redis.hgetall("metrics:health"),
        this.redis.hgetall("perf:health"),
