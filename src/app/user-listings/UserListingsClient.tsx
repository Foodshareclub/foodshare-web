...
export function UserListingsClient({ listings, user }: UserListingsClientProps) {
  ...
  const handleRefresh = async () => {
    ...
    try {
      await refreshUserListingsCache(user.id);
      router.refresh();
    } finally {
  ...
  const filteredListings = listings
    .filter((listing) => {
      ...
      if (filterStatus === "active" && listing.is_active !== true) return false;
      if (filterStatus === "inactive" && listing.is_active !== false) return false;
  ...
  // Explicitly check for boolean true/false to handle potential null values
  const activeListings = listings.filter((l) => l.is_active === true);
  ...
  const activeFilterCount = [searchQuery, filterStatus !== "all", filterType !== "all"].filter(
    Boolean
  ).length;
          ...
          <div className="relative overflow-hidden rounded-2xl border border-border/40 bg-card/80 backdrop-blur-xl p-6 shadow-sm hover:shadow-lg transition-shadow duration-300">
            ...
            <div className="relative">
              ...
              <div className="mb-6">
                ...
                <p className="text-sm text-muted-foreground">Manage your shared items and food</p>
              </div>
            ...
            <div className="flex flex-wrap items-center gap-3">
                ...
                <SheetTrigger asChild>
                  ...
                  >
                    ...
                    {hasActiveFilters && (
                      <Badge
                        variant="secondary"
                        className="h-5 px-1.5 text-xs bg-emerald-100 text-emerald-700 dark:bg-emerald-900/50 dark:text-emerald-300"
                      >
                        {activeFilterCount}
                      </Badge>
                    )}
              ...
              {postTypes.length > 0 && (
                <DropdownMenu>
                  ...
                  <DropdownMenuContent
                    align="start"
                    className="w-52 p-2 animate-in fade-in-0 zoom-in-95 slide-in-from-top-2"
                  >
                    ...
                    >
                      ...
                      {filterType === "all" && <Check className="h-4 w-4 text-emerald-600" />}
                    </DropdownMenuItem>
                    {postTypes.map((type) => (
                      <DropdownMenuItem
                        key={type}
              ...
              <DropdownMenu>
                ...
                <DropdownMenuContent
                  align="start"
                  className="w-48 p-2 animate-in fade-in-0 zoom-in-95 slide-in-from-top-2"
                >
                  <DropdownMenuLabel className="text-xs text-muted-foreground px-2">
                    Filter by status
                  </DropdownMenuLabel>
              ...
              <DropdownMenu>
                <DropdownMenuTrigger asChild>
                  ...
                  >
                    <span>
                      {currentSort?.icon} {currentSort?.label.split(" ")[0]}
                    </span>
                    <ChevronDown className="h-4 w-4 opacity-60" />
                  </Button>
                </DropdownMenuTrigger>
                ...
                >
                  ...
                  {SORT_OPTIONS.map((option) => (
                    ...
                    >
                      <span className="text-base">{option.icon}</span>
                      <span className="flex-1 font-medium">{option.label}</span>
                      {sortBy === option.value && <Check className="h-4 w-4 text-emerald-600" />}
                    </DropdownMenuItem>
                  ))}
                </DropdownMenuContent>
              </DropdownMenu>
              ...
              <div className="relative overflow-hidden rounded-3xl border border-border/40 bg-card/80 backdrop-blur-xl p-8 sm:p-12 text-center max-w-md mx-4 shadow-lg hover:shadow-xl transition-shadow duration-300">
                ...
                <div className="relative">
                  ...
                  <Link href="/food/new">
                    <Button
                      size="lg"
                      className="h-12 px-6 sm:px-8 rounded-xl bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-700 hover:to-teal-700 shadow-lg shadow-emerald-500/25 transition-all duration-300 hover:shadow-xl hover:shadow-emerald-500/30 hover:scale-105 gap-2"
                    >
              ...
              {filteredListings.map((listing) => (
                <div
                  ...
                  className={cn(
                    ...
                    !listing.is_active &&
                      "opacity-50 grayscale-[30%] hover:opacity-70 hover:grayscale-0"
                  )}
                >
