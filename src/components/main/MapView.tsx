...
export function MapView({ products, hoveredProductId, selectedProductId, onMarkerClick }: MapViewProps) {
    ...
    useEffect(() => {
      ...
      products.forEach((product) => {
        // Note: products should have coordinates from distance worker
        // which normalizes PostGIS location to lat/lng
        if (!(product as any).latitude || !(product as any).longitude) return;
        ...
        if (existingMarker) {
          ...
          if (
            currentLatLng.lat !== (product as any).latitude ||
            currentLatLng.lng !== (product as any).longitude
          ) {
            existingMarker.setLatLng([(product as any).latitude, (product as any).longitude]);
          }
        } else {
          // Create new marker with interactive product card popup
          const marker = L.marker([(product as any).latitude, (product as any).longitude], {
            icon: defaultIcon,
          })
            .addTo(map)
