 ...
 * Fetches auth and chat data on the server and passes to client Header
 */
...
export default async function HeaderServer({
  ...
  // Fetch profile and chat rooms in parallel if authenticated
  let profile = null;
  let chatRooms: Awaited<ReturnType<typeof getUserChatRooms>> = [];

  if (userId) {
    [profile, chatRooms] = await Promise.all([
      getProfile(userId),
  ...
  const avatarUrl =
    profile?.avatar_url ||
    authSession.user?.profile?.avatar_url ||
    '';
