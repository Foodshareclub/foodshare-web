...
describe("Edge Function transport", () => {
  test("includes authentication, gateway key, and lossless query parameters", async () => {
    ...
    const result = await apiGet("api-v1-products", { lat: 0, lng: 0, active: false, cursor: undefined });
  ...
  test("accepts a successful response with no content", async () => {
    ...
    expect(await apiCall("api-v1-products", { method: "DELETE" })).toEqual({ success: true, data: undefined });
  });
  ...
  test("does not accept a success envelope with a failing HTTP status", async () => {
    ...
    expect(await apiGet("api-v1-products")).toMatchObject({ success: false, error: { code: "INTERNAL_ERROR" } });
  });
  ...
  test("clears the timeout after a network failure", async () => {
    ...
    expect(await apiGet("api-v1-products")).toMatchObject({ success: false, error: { code: "NETWORK_ERROR" } });
