...
const updateProduct = mock(
  async (_id: number, _form: FormData): Promise<ActionResult<undefined>> => ({ success: true, data: undefined }),
);
const updateRoom = mock(async (_id: string, _form: FormData) => ({ success: true, data: undefined }));
const refresh = mock(() => {});
...
mock.module("@/components/modals/AuthenticationUser/AuthenticationUserModal", () => ({ default: () => null }));
...
describe("handover confirmation", () => {
  test("sends the loaded revision and does not arrange a room after a failed listing update", async () => {
    ...
    const view = render(
      <OneProduct product={product} buttonValue="approval pending" roomId="room-1" requesterId="requester-1" />,
    );
    fireEvent.click(view.getByRole("button", { name: "approval pending" }));
    await waitFor(() => expect(view.getByRole("alert").textContent).toContain("This listing changed"));
