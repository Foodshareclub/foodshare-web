...
const createProductAPI = mock(
  async (_input: unknown): Promise<ActionResult<{ id: number; post_type: string; is_active: boolean }>> => ({
    success: true,
  ...
  from: () => {
    ...
    const resolve = async () => {
      if (data) {
        ...
        return { data: insert || filters.version === state.revision ? { id: 42 } : null, error: null };
      }
      state.reads++;
...
mock.module("@/lib/data/cache-invalidation", () => ({ invalidateTag, invalidatePostActivityCaches: mock() }));
mock.module("@/app/actions/analytics", () => ({ trackEvent: mock(async () => {}) }));
mock.module("@/app/actions/post-activity", () => ({ logPostActivity: mock() }));
mock.module("@/lib/storage/search", () => ({ indexProduct: mock(async () => true), removeProductFromSearch: mock() }));
...
describe("listing mutation actions", () => {
  ...
  it("records category corrections as manual decisions and preserves other metadata", async () => {
    ...
    expect(state.writes[0].data).toMatchObject({
      post_type: "thing",
      category_id: null,
      metadata: { importSource: "legacy", classification: { source: "manual", postType: "thing", needsReview: false } },
    });
  });
});
