...
export function useDecorativeShader(loadShader: ShaderLoader, label: string, scroll: number) {
  ...
  useEffect(() => {
    ...
    async function start() {
      starting = true;
      try {
        const [{ effect, frameLoop, clock }, shader] = await Promise.all([import("vgpu"), loadShader()]);
        if (cancelled) return;
        render = await initGPURender({ canvasRef, signal: controller.signal, surfaceOptions: { dpr: [1, 1.5] } });
        if (cancelled) {
          render.dispose();
          return;
        }
        ...
        render.onCleanup(surface.onResize(() => decoration.set({ params: { resolution: surface.size } })));
