/* @ts-self-types="./foodshare_geo.d.ts" */
 ...
 * @returns {any}
 */
function calculate_distances_sorted(user_lat, user_lng, products, max_results) {
  try {
    const retptr = wasm.__wbindgen_add_to_stack_pointer(-16);
    wasm.calculate_distances_sorted(
      retptr,
      user_lat,
      user_lng,
      addHeapObject(products),
      max_results
    );
...
exports.calculate_distances_sorted = calculate_distances_sorted;
 ...
 * @returns {any}
 */
function calculate_product_distances(user_lat, user_lng, products) {
  try {
    const retptr = wasm.__wbindgen_add_to_stack_pointer(-16);
    wasm.calculate_product_distances(retptr, user_lat, user_lng, addHeapObject(products));
    var r0 = getDataViewMemory0().getInt32(retptr + 4 * 0, true);
    var r1 = getDataViewMemory0().getInt32(retptr + 4 * 1, true);
    var r2 = getDataViewMemory0().getInt32(retptr + 4 * 2, true);
    if (r2) {
...
exports.calculate_product_distances = calculate_product_distances;
...
function distance(lat1, lng1, lat2, lng2) {
  const ret = wasm.distance(lat1, lng1, lat2, lng2);
  return ret;
}
 ...
 * @returns {any}
 */
function filter_within_radius(user_lat, user_lng, products, radius_km) {
  try {
    const retptr = wasm.__wbindgen_add_to_stack_pointer(-16);
    wasm.filter_within_radius(retptr, user_lat, user_lng, addHeapObject(products), radius_km);
    var r0 = getDataViewMemory0().getInt32(retptr + 4 * 0, true);
    var r1 = getDataViewMemory0().getInt32(retptr + 4 * 1, true);
    var r2 = getDataViewMemory0().getInt32(retptr + 4 * 2, true);
    if (r2) {
...
exports.filter_within_radius = filter_within_radius;
...
function parse_location(location) {
  try {
    const retptr = wasm.__wbindgen_add_to_stack_pointer(-16);
    wasm.parse_location(retptr, addHeapObject(location));
    var r0 = getDataViewMemory0().getInt32(retptr + 4 * 0, true);
    var r1 = getDataViewMemory0().getInt32(retptr + 4 * 1, true);
    var r2 = getDataViewMemory0().getInt32(retptr + 4 * 2, true);
    if (r2) {
...
function __resolveWasmPath(filename) {
  ...
  const candidates = [
    ...
    path.join(process.cwd(), "src", "wasm", "foodshare-geo", filename),
    path.join(process.cwd(), "foodshare-web", "src", "wasm", "foodshare-geo", filename),
    path.join(process.cwd(), "..", "foodshare-web", "src", "wasm", "foodshare-geo", filename),
  ];
...
const wasmPath = __resolveWasmPath("foodshare_geo_bg.wasm");
